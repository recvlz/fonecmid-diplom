...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТЗУдержания.Колонки.Добавить("Сотрудник");
...
//				ДвижениеНДФЛ.БазовыйПериодКонец = КонецМесяца(Дата);
				ТЗУдержания.Добавить().Сотрудник = Выборка.Сотрудник;
...
//Расчет движений: Расчет отпуска и НДФЛ от отпуска
	Движения.ВКМ_ОсновныеНачисления.Записывать = Истина;	
	Запрос = Новый Запрос;
	Запрос.Текст ="ВЫБРАТЬ ВКМ_ОсновныеНачисления.НомерСтроки,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ОсновныеНачисления.БазаВКМ_ОсновныеНачисления(&Измерения, &Измерения, &Разрезы,
	...
	|			&Разрезы, Регистратор = &Регистратор
	...
	Пока Выборка.Следующий() Цикл
		Движение = Движения.ВКМ_ОсновныеНачисления[Выборка.НомерСтроки - 1];
		Движение.Дней = Выборка.ОтпускныхДней;
		
		Если Выборка.ДнейБаза = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Движение.Сумма = (Выборка.СуммаБазаОсн + Выборка.СуммаБазаДоп) * Выборка.ОтпускныхДней / Выборка.ДнейБаза;	
...
//Создание движений: Удержания
	ТЗУдержания.Свернуть("Сотрудник");
	
	Для Каждого СтрокаТЗ Из ТЗУдержания Цикл
			ДвижениеНДФЛ = Движения.ВКМ_Удержания.Добавить();
			ДвижениеНДФЛ.Сотрудник = СтрокаТЗ.Сотрудник;
			ДвижениеНДФЛ.ПериодРегистрации = Дата;
			ДвижениеНДФЛ.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.НДФЛ;
			ДвижениеНДФЛ.БазовыйПериодНачало = НачалоМесяца(Дата);
			ДвижениеНДФЛ.БазовыйПериодКонец = КонецМесяца(Дата);
...
//Расчет движений: Удержания	
	...
	|	ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.СуммаБаза, 0) КАК СуммаОсн,
	|	ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.СуммаБаза, 0) КАК СуммаДоп,
	...
	Пока Выборка.Следующий() Цикл
		...
		ДвижениеНДФЛ.ОснНач = Выборка.СуммаОсн;
		ДвижениеНДФЛ.ДопНач = Выборка.СуммаДоп;
		ДвижениеНДФЛ.Сумма = (Выборка.СуммаОсн + Выборка.СуммаДоп)*Выборка.Значение / 100;
	КонецЦикла;
