...
//Создание движений: Регистр ВКМ_СтоимостьИТУслуг
	Движения.ВКМ_СтоимостьИТУслуг.Записывать = Истина;
	НоменАбонПлата = Константы.ВКМ_НоменклатураАбонентскаяПлата.Получить();
	НоменРаботы = Константы.ВКМ_НоменклатураРаботыСпециалиста.Получить();
	Для Каждого ТекСтрокаУслуги Из Услуги Цикл
		Если ТекСтрокаУслуги.Номенклатура = НоменАбонПлата Тогда
			Движение = Движения.ВКМ_СтоимостьИТУслуг.Добавить();
			Движение.Период = Дата;
			Движение.Контрагент = Контрагент;
			Движение.Договор = Договор;
			Движение.СуммаАбонПлата = ТекСтрокаУслуги.Сумма;
		КонецЕсли;
		Если ТекСтрокаУслуги.Номенклатура = НоменРаботы Тогда
			Движение = Движения.ВКМ_СтоимостьИТУслуг.Добавить();
			Движение.Период = Дата;
			Движение.Контрагент = Контрагент;
			Движение.Договор = Договор;
			Движение.СуммаРаботыСпеца = ТекСтрокаУслуги.Сумма;
		КонецЕсли;
	КонецЦикла;
//Конец ВКМ_Доработки
КонецПроцедуры
