...
Функция  СоздатьАктыНаСервере(Период) Экспорт 
	...
	Для Каждого СтрокаТЗ Из Таблица Цикл 
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЗ.Реализация) Тогда 
			...
			СозданныйДокумент = СоздатьДокументРеализации(СтрокаТЗ, КонецМесяца(Период));
			
			Если СозданныйДокумент <> Ложь Тогда
				МассивДляТЧ.Добавить(Новый Структура("Договор, Реализация", СтрокаТЗ.Договор, СозданныйДокумент));
			КонецЕсли;
